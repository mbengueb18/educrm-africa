...
export function CampaignsClient({ campaigns, stages, programs }: CampaignsClientProps) {
  ...
  var [confirmSendCampaign, setConfirmSendCampaign] = useState<Campaign | null>(null);
  var [confirmStats, setConfirmStats] = useState<{
    total: number;
    withEmail: number;
    withoutEmail: number;
  ...
  // Ouvre le modal de confirmation et charge les stats
  var handleSend = async function(campaign: Campaign) {
  ...
  // Confirme et envoie réellement
  var handleConfirmSend = function() {
    if (!confirmSendCampaign) return;
    var campaign = confirmSendCampaign;
    setConfirmSendCampaign(null);
    setConfirmStats(null);
    startTransition(async function() {
      try {
