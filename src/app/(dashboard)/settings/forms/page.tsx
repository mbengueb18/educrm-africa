...
import {
  getFormsWithMapping, deleteFormSchema,
  type FormWithMapping, type MappedField,
} from "@/lib/form-schemas";
import { toast } from "sonner";
import {
  ArrowLeft, ChevronDown, ChevronRight, FileText, CheckCircle2,
  AlertCircle, Calendar, Layers, Settings2, Trash2, Search, ExternalLink, Loader2,
} from "lucide-react";
    ...
    <div>
            ...
            return (
              <div key={form.formId} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-3 sm:px-4 py-3.5 hover:bg-gray-50/70">
                  ...
                  <button
                    onClick={() => handleDelete(form.formId, form.name)}
                    disabled={isPending}
                    className="shrink-0 p-1.5 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"
      ...
      {forms.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-xs text-blue-800 mt-6">
          ...
          <p>
            Cette page liste les formulaires détectés sur votre site et indique, pour chaque champ, s&apos;il alimente
            le CRM. Les champs <strong>auto</strong> (prénom, nom, email, téléphone) sont reconnus automatiquement.
            Pour mapper un champ non reconnu, rendez-vous sur la page <Link href="/settings/custom-fields" className="underline">Champs personnalisés</Link>.
          </p>
        </div>
      )}
    </div>
...
// ─── Une ligne de champ (lecture seule) ───
