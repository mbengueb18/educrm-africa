...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(new URL("/settings/integrations?gmail=error&reason=" + error, request.url));
  }

  if (!code || !state) {
    return NextResponse.redirect(new URL("/settings/integrations?gmail=error&reason=missing_params", request.url));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/settings/integrations?gmail=error&reason=invalid_state", request.url));
  }

  var clientId = process.env.GOOGLE_CLIENT_ID!;
  var clientSecret = process.env.GOOGLE_CLIENT_SECRET!;
  var redirectUri = process.env.GMAIL_REDIRECT_URI!;
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(new URL("/settings/integrations?gmail=error&reason=token_exchange", request.url));
  }
